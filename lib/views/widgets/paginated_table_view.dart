...
class _PaginatedTableViewState<T> extends State<PaginatedTableView<T>> {
  ...
  void initState() {
    ...
    ever<List<T>>(widget.items as RxInterface<List<T>>, (list) {
      // updateAll expects a List<T>
      _source.updateAll(List<T>.from(list));
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isLoading == true && widget.hasError!.isNotEmpty) {
      debugPrint("Error: ${widget.hasError}");
      return Container(
    ...
    return SingleChildScrollView(
      child: PaginatedDataTable(
        ...
        columns:
            List.generate(widget.columns.length, (idx) {
